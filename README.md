...
1. Run `build_conflicts_d.py` which takes as implicitly takes as input `student_schedule_d.pkl` (generated by `build_student_schedule.py`). This outputs the file `conflicts_d.pkl` which contains counts of conflicting course pairs.
...
1. Run `build_no_lunch_d.py` which takes as implicitly takes as input `student_schedule_d.pkl` (generated by `build_student_schedule.py`). This outputs the file `no_lunch_d.pkl` which contains counts of students that do not have time for lunch.
