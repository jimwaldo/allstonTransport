...
1. First, clean `enrollment.csv` and `course_times.csv` with `clean_csv.py`.

2. Run `split_by_term.py` which will split the enrollment and course time files into directories, one for each term, containing an enrollment.csv and course_times.csv file for the 
courses taught that term.

3. Run `split_classes_students.py`. This takes as input the `enrollment.csv` file. It uses the file `allston_course_selector.py` to determine which classes will be taught in Allston. This will produce three files:  
...
4. Run `build_course_times.py`, which builds a dictionary of courses and class times taking as input the file `course_times.csv` supplied by the registrar, `all_allston_student_classes_set.pkl` (the set of all classes taken by students who take any class in Allston produced by `split_classes_students.py`, and `Allston_class_set.pkl` (the set of all courses taught in Allston produced by `split_classes_students.py`). This will generate `class_time_d.pkl`, a dictionary indexed by class with value a `course_time` object that has the course, time at which the course meets, where the course meets, and the days of the week that the course meets.

5. Run `student_schedule.py`, which builds a dictionary, indexed by student, of the schedule for each student that takes a course that meets in Allston. The program takes as input `Allston_student_set.pkl` (the set of students who take a course in Allston, generated by `split_classes_students.py`), the `enrollments.csv` file supplied by the registrar, and `class_time_d.pkl` (the dictionary of class times generated by `build_course_times.py`). It produces the file `student_schedule_d.pkl`.

6. Run `build_transition_d.py`, which takes as input `student_schedule_d.pkl` (generated by `student_schedule.py`). This produces a dictionary keyed by student id that shows the transitions across the river for each time and day
for that student, written to the file `transition_d.pkl`

7. Finally, run `build_transition_time_d.py` that will build a list of dictionaries, one for each day, keyed by time with a count of the number of transitions from Cambridge to Allston and from Allston to Cambridge on that day and time. This takes as input `transition_d.pkl` (generated by `build_transition_d.py`) and produces a file `trans_time_d_l.pkl`.
