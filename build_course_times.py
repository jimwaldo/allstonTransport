...
Build a dictionary keyed by class number with values a list of course time objects, which contain the class number, the start
...
def build_ct_d(csv_in, convert_to_allston):
    ...
    for l in csv_in:
        ...
        if l[1] in ret_d:
            # uhoh, multiple coursetimes...
            ret_d[l[1]].append(cto)
            warnings.warn("Course %s has multiple times: %s"%(course_name,";".join(map(str,ret_d[l[1]]))))
        else:
            ret_d[l[1]] = [cto]
